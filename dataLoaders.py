...
class DigitData(Dataset):
    def __init__(self, data_frame=None, label=None, pixel_col=None, reshape_pixel=None, path=None):
        if data_frame is None: data_frame = pd.read_csv(path)  # ,nrows=10000
    ...
    def __getitem__(self, idx):
        ...
        #pixel = torch.where(pixel > torch.tensor(0), torch.tensor(255.0), torch.tensor(0.0))  # add step to preprocess the data
