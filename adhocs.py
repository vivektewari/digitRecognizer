...
file=pd.read_csv(devData,nrows=5000)[[localization_col]]

temp=np.array(list(file[localization_col ].apply(lambda x:packing.unpack(x))))
#temp=vison_utils.xy_to_cxcy(torch.Tensor(temp)/112)
temp=vison_utils.get_scale_aspect(xy=torch.tensor(temp/112))
temp=torch.stack(list(temp), dim=1)
#clusterring.kmeans(temp,standardized=True) # selected 10 clusters
centroids = clusterring.kmeans(temp,n_cluster=7,standardized=True)
