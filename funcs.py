...
def toImage(read_path,save_path,image_dim):
    train = pd.read_csv(read_path, nrows=500)
    plt.gray()
    for row in range(train.shape[0]):
        ...
        data = data.reshape(image_dim)
        fig = plt.figure()
        plt.imshow(data)
        fig.savefig(save_path +'/'+ str(str(row) + "_" + str(label) + '.png'))
        plt.close(fig)
...
def getMetrics(actual, predicted):
    ...
    actual_=torch.as_tensor(actual).clone().detach().to('cpu')
    predicted_ = torch.as_tensor(predicted).clone().detach().to('cpu')
    for m in metrics:
        if m == accuracy_score:
            met.append(m(actual_, predicted_))
        else:
            met.append(m(actual_, predicted_, average="macro"))
    return met
...
class DataCreation:
    ...
    def shifter(self, data, data_count=100, size=(112, 112),size2=(28,28),chunk_size=None):
        pixel = ['pixel' + str(i) for i in range(size[0] * size[1])]
        pixel2 = ['pixel' + str(i) for i in range(size2[0] * size2[1])]
        if chunk_size == None :
            chunk_size =data_count
        chunk = 0
        while chunk < data_count:
            data_big = self.black_image(chunk_size,size)
            for i in range(chunk_size):
                ...
                temp[start_row:start_row + size2[0], start_col:start_col + size2[1]] = \
                    np.array(data.iloc[i+chunk][pixel2]).reshape(size2)
                data_big.iloc[i][pixel] = temp.flatten()
                data_big.iloc[i]['label'] = data.iloc[i+chunk]['label']
                self.get_fig(temp, i+chunk, data.iloc[i+chunk]['label'])
            if self.to_csv:
                    if chunk == 0 : data_big.to_csv(str(self.data_path) + '/newData.csv')
                    else :data_big.to_csv(str(self.data_path) + '/newData.csv',mode= 'a',header=False)
            chunk += chunk_size
            if chunk+ chunk_size>data_count : chunk_size = data_count - chunk




        if not self.to_csv :return data_big

    def scaler(self, data, data_count=100, size=(112, 112),size2=(28,28),scales=4,chunk_size=None):
        ...
        while chunk < data_count :
            ...
            for i in range(chunk_size):
                ...
                res = cv2.resize(np.array(data.iloc[i+chunk][pixel2]).reshape(size2).astype('float32')
            ...
            if self.to_csv:
                    if chunk == 0:
                        ...
                        data_big.to_csv(str(self.data_path) + '/newData.csv', mode='a', header=False)
            chunk += chunk_size
            if chunk + chunk_size > data_count: chunk_size = data_count - chunk
        if not self.to_csv: return data_big
